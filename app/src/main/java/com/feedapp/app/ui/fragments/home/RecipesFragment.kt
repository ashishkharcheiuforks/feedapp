...
class RecipesFragment () : DaggerFragment(), OnSearchLimit, SearchRecipeLoadException {
    ...
    override fun onCreateView(
        ...
        resultAdapter = RecipesRecyclerAdapter(activity!!, requestManager, this)
    ...
    private fun scrollActivityToTop() =
        try {
            activity!!.findViewById<NestedScrollView>(R.id.activity_main_nsv).smoothScrollTo(0, 0)
    ...
    private val searchActionListener = object : MaterialSearchBar.OnSearchActionListener {
        ...
        override fun onSearchConfirmed(text: CharSequence?) {
            ...
            if (viewModel.isConnected.value != null && viewModel.isConnected.value!!) {
                if (!viewModel.ifRecipesLimitReached()) {
                    viewModel.isSearching.postValue(true)
                    viewModel.hasSearched.postValue(true)
                    viewModel.searchRecipe(text.toString(), this@RecipesFragment)
    ...
    private fun searchLimitReached() {
        AlertDialog.Builder(activity!!)
            .setTitle(getString(R.string.dialog_search_limit))
        ...
        viewModel.run {
            ...
            searchRecipes.observe(viewLifecycleOwner, Observer {
                if (it.results.isNullOrEmpty()) {
                    viewModel.isRecipesEmpty.postValue(true)
                } else {
                    viewModel.isRecipesEmpty.postValue(false)
                }
    ...
    override fun ifLimitReached(): Boolean = viewModel.ifRecipesLimitReached()
