...
class SecIntro : Fragment(), ISlidePolicy {
    ...
    private fun setListeners(view: View) {
        ...
        spinnerExc.setOnItemClickListener { _, _, position, _ ->
            ...
            introViewModel.activityChosen.value = true
        ...
        spinnerSex.setOnItemClickListener { _, _, position, _ ->
            activity?.hideKeyboard()
            introViewModel.sex.value = position == 0
            introViewModel.sexChosen.value = true
            unblockButton()
        }

        intro_sec_continue.setOnClickListener {
            introViewModel.applied.postValue(true)
        }
        edt_height.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                if (edt_height.text.length >= 2) {
                    introViewModel.height.value = edt_height.text.toString().toInt()
                    unblockButton()
                }
        ...
        edt_years.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                if (s.toString().isNotEmpty()) {
                    introViewModel.age.value = edt_years.text.toString().toInt()
                    unblockButton()
                }
            }
        })
        edt_weight.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                if (s.toString().isNotEmpty()) {
                    introViewModel.weight.value = edt_weight.text.toString().toInt()
                    unblockButton()
                }
            }
        })
    ...
    fun unblockButton() {
        if (introViewModel.canUnblockButton(
            ...
            if (introViewModel.areHeightWeightValid(
                    edt_height.text.toString(),
                    edt_weight.text.toString()
                )
            ) {
                intro_sec_continue.isEnabled = false
                if (introViewModel.toastShown.value == false) {
                    activity?.toastLong(getString(R.string.incorrect_data))
                    introViewModel.toastShown.value = true
                }
                return
            }
